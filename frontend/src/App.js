...
export default function App() {
  ...
  const changeNavbarToAdmin = () =>{
    setChooseNavbar(2)
  }
  const changeNavbarToUser = () =>{
    setChooseNavbar(1)
  }
  ...
  return (
    <BrowserRouter>
      ...
      {(chooseNavbar===1) ? <Navbar size={cart.length}/>:(chooseNavbar===2)?<NavbarAdmin />:null}
      
      
      {/* <NavbarAdmin/> */}
      
      <Routes>
        <Route exact path='/' element={<LoginPage changeNavbarToAdmin={changeNavbarToAdmin} changeNavbarToUser={changeNavbarToUser}/>} />
        <Route exact path="/user/about" element={<div></div>} />
        <Route exact path='/user/services' element={<div></div>} />
        <Route exact path="/user/home" element={<Home />} />
        <Route exact path='/user/menu' element={<MenuPage handleClick={handleClick} cart={cart}/> } />
